...
struct hash_table_v2 *hash_table_v2_create()
{
	...
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		...
		//init. lock per entry (bucket) 
		if(pthread_mutex_init(&entry->mutexV2, NULL) != 0)
			exit(EXIT_FAILURE); 
	}
	return hash_table;
}
                             ...
                             uint32_t value)
{
	...
	struct hash_table_entry *hash_table_entry = get_hash_table_entry(hash_table, key);
	 ...
	 // lock only mutex associated with specific bucket
    if(pthread_mutex_lock(&hash_table_entry->mutexV2) != 0) {
        exit(EXIT_FAILURE); 
    }
	...
	if (list_entry != NULL) {
		...
		//unlock mutex 
		if(pthread_mutex_unlock(&hash_table_entry->mutexV2) != 0){
			exit(EXIT_FAILURE);
		}
		return;
	}
...
void hash_table_v2_destroy(struct hash_table_v2 *hash_table)
{
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		...
		//destroy locks corresponding to each bucket in hash table
		if(pthread_mutex_destroy(&entry->mutexV2) != 0)
			exit(EXIT_FAILURE); 
